...
WORK = dict(synthetic_pairs=10, synthetic_episodes=20, synthetic_queries=120,
            score_calls=0, mock_pair_admissions=0, mock_text_packings=0)


def _fixtures():
    rows=[]
    for index,(contrast,panel) in enumerate((c,p) for c in range(5) for p in evaluation.PANELS):
        for variant in (0,1):
            ...
            rows.append(dict(id=f"row{index}-{variant}",counterfactual_group=f"pair{index}",
                ...
                recipe=dict(base_pair_sha256=f"base{index}",base_ids=[f"base{index}-0",f"base{index}-1"],
                    contrast_index=contrast,contrast_codes=list(evaluation.CONTRASTS[contrast])),
...
class ComplementaryCompositionEvaluationTests(unittest.TestCase):
    def test_two_panels_five_contrasts_and_entire_paired_query_inventory(self):
        result=score(records());c=result["overall"]["counts"]
        self.assertEqual(result["overall"]["query_turns"],120)
        self.assertEqual(c[evaluation.PRIMARY_METRIC],dict(count=10,total=10,rate=1.0))
        self.assertEqual(set(result["by_panel"]),set(evaluation.PANELS))
        self.assertEqual(c["known_both"]["count"],100)
        self.assertEqual(c["unknown_both"]["count"],20)
        self.assertEqual(set(result["by_contrast"]),{"/".join(c) for c in evaluation.CONTRASTS})
    ...
    def test_second_word_free_reply_failure_defeats_pair_despite_anchor(self):
        ...
        self.assertEqual(c["anchor_pair_both"]["count"],10)
        self.assertEqual(c["all_query_pair_action"]["count"],10)
        self.assertEqual(c["all_query_pair_reply"]["count"],9)
        self.assertEqual(c[evaluation.PRIMARY_METRIC]["count"],9)
    ...
    def test_application_version_includes_unchanged_roles(self):
        ...
        self.assertEqual(c["revision_both"],dict(count=30,total=30,rate=1.0))
        self.assertEqual(c["prior_value_both"]["count"],20)
    ...
    def test_reply_bytes_after_eos_are_not_exact_and_action_ties(self):
        ...
        c=score(raw)["overall"]["counts"]
        self.assertEqual(c["anchor_pair_action"]["count"],10)
        self.assertEqual(c["anchor_pair_reply"]["count"],9)
    ...
    def test_mocked_admission_then_text_only_packing_and_immutable_inventory(self):
        ...
        self.assertEqual(events,["admit"]*10+["pack"]+["admit"]*10+["pack"])
    ...
    def test_contrast_identity_and_honest_train_fit_role(self):
        raw=records();expected=dict(query_inventory=evaluation._inventory(raw))
        for row in raw[:2]:
            row["contrast_index"]=1;row["contrast_codes"]=list(evaluation.CONTRASTS[1])
        with self.assertRaisesRegex(ValueError,"complete query inventory"):score(raw,expected_bank=expected)
        raw=records();raw[0]["contrast_codes"].reverse()
        with self.assertRaisesRegex(ValueError,"record identity"):score(raw)
        rows=deepcopy(ROWS[:2])
        for row in rows:row["split"]="train"
        def pack(*args,**kwargs):
            WORK["mock_text_packings"]+=1;return {}
        with patch.object(evaluation.curriculum,"validate_pair",side_effect=admit),patch.object(evaluation,"pack_observations",side_effect=pack):
            bank=evaluation.PreparedDefinitionBank(rows,role="train_fit")
            self.assertEqual(bank.identity["role"],"train_fit")
            self.assertEqual(bank.identity["query_inventory"][0]["contrast_codes"],list(evaluation.CONTRASTS[0]))
        with patch.object(evaluation,"pack_observations") as packed:
            with self.assertRaisesRegex(ValueError,"explicit train_fit/dev"):evaluation.PreparedDefinitionBank(rows,role="audit")
