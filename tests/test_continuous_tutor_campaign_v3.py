...
class RecoveryTests(unittest.TestCase):
    def setUp(self):
        fixture.SOURCE = ROOT/"experiments/continuous_tutor_campaign_v3.py"
        self.h = fixture.CampaignTests("runTest"); self.h.setUp(); self.addCleanup(self.h.doCleanups)
        self.owner = self.h.owner
        self.state = self.h.state('authored'); self.state['schema'] = 'bic-continuous-tutor-campaign-v2'
        self.state.update(cycle=2,completed_cycles=1)
        self.state['parent']['lifetime_updates']=7816
        self.state['physical_totals']=dict(worker_updates=432,teacher_chat_attempts=2,teacher_chat_completions=2)
        self.parent = deepcopy(self.state['parent'])
        self.context = dict(parent=dict(identity_sha256=self.parent['checkpoint']['sha256'],
            weights_sha256=self.parent['weights_sha256'], cycle=2,lifetime_updates=self.parent['lifetime_updates']),
            teaching_contract=dict(identity_sha256='c'*64))
        self.state['cycle_context'] = self.h.record('context.json', self.context)
        self.preparation = dict(status='failed',error=self.owner.FINITE_VARIATION_ERROR,
            compiler_attempts=2,compiler_completions=1,compiler_receipts=[dict(status='completed'),
                dict(status='failed',error='fresh realization exhausted 64 candidates: slot=105, offset=14, family=switch, realization=revalue')],
            neural_work=0,teacher_calls=0,wall_seconds=100.,cpu_seconds=99.)
        ...
        self.previous = dict(schema='bic-continuous-tutor-campaign-v2',budget_seconds=3469,
        ...
        self.summary = dict(status='failed',error=self.owner.FINITE_VARIATION_ERROR,partial_work_unknown=False,
            worker_processes=[dict(directory='prior-worker',launch_sha256='8'*64,exit_code=0)],physical_totals=deepcopy(self.state['physical_totals']),
            parent=deepcopy(self.parent),active_stage=dict(stage='authored',cycle=2),final_stage='authored',wall_seconds=460.953,cpu_seconds=450.)
    ...
    def test_exact_known_failure_admission_is_detached(self):
        ...
        self.assertEqual(restored['physical_totals']['teacher_chat_completions'],2)
        self.assertEqual(restored['physical_totals']['worker_updates'],432)
    ...
    def test_unknown_worker_started_or_foreign_failure_refused(self):
        cases=(('partial_work_unknown',True),('worker_processes',[{'pid':1}]),
            ...
            ('active_stage',dict(stage='compiled',cycle=2)),('active_stage',dict(stage='authored',cycle=1)))
        for key,value in cases:
            with self.subTest(key=key,value=value):
                changed=deepcopy(self.summary);changed[key]=value
                with self.assertRaises(ValueError):self.admit(summary=changed)
        for field,value in (('worker_updates',1),('teacher_chat_attempts',1),('teacher_chat_completions',0)):
    ...
    def test_parent_and_complete_failed_compiler_bindings(self):
        ...
        for field,value in (('status','completed'),('error','different'),('compiler_completions',2),
                ('compiler_receipts',[dict(status='completed'),dict(status='completed')]),('neural_work',1),
                ('teacher_calls',1)):
    ...
    def test_remaining_budget_debits_failed_work_without_rounding_up(self):
        for spent,expected in ((460.953,3008),(460.,3009),(3349.,120)):
            changed=deepcopy(self.summary);changed['wall_seconds']=spent
            self.assertEqual(self.admit(summary=changed)[1],expected)
        for spent in (0,-1,float('nan'),float('inf'),True,3349.001,3469,3470):
            changed=deepcopy(self.summary);changed['wall_seconds']=spent
    ...
    def test_freeze_reuses_exact_teacher_and_pins_all_prior_work(self):
        ...
        self.assertEqual(launch['budget_seconds'],3008)
        ...
        self.assertEqual(state['cycle_directory'],'recovery/execution/cycles/000002')
        self.assertEqual(state['reuse_procedural']['preparation_sha256'],self.preparation_record['sha256'])
        self.assertEqual(launch['recovery']['wall_seconds'],460.953)
