...
from experiments import immutable_lesson_cache as cache
...
WORK = dict(synthetic_images=0, synthetic_json_bytes=0, callback_attempts=0,
            callback_completions=0, schedule_simulations=0)
CACHES, RETAINED = [], []
...
class CacheTests(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='fixtures-', dir=ROOT/'runs/immutable-lesson-cache-validation-local')
    ...
    def make(self, *, records=None, callback=admission, **kwargs):
        ...
        value = cache.ImmutableLessonCache(ROOT,records=records,records_sha256=cache.identity(records),
    ...
    def test_detached_exact_snapshot_and_admission_once(self):
        ...
        self.assertEqual(after['admissions'],1);self.assertEqual(after['decodes'],1)
