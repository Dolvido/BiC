...
class VerifiedTutorAuthorTests(unittest.TestCase):
    ...
    def request(self, mode="local", chapters=2):
        contract = dict(schema="bic-verified-tutor-curriculum-contract-v1",
            inventory_sha256="a"*64, protected_sha256="b"*64, protected_count=1,
            seed=852620001, start_cursor=1944, micro_batch_size=2, layout="original",
            chapters=[dict(motifs=["pool_a", "pool_b"], realizations=["independent", "rename", "revalue"],
                           default=dict(motif="pool_a", realization="independent")) for _ in range(chapters)],
            slots=[dict(chapter=c, depth=d, turns=t, replay=False)
                   for c in range(chapters) for d in range(6) for t in (8,10,12)],
            rehearsal_floor_by_depth={str(d):0 for d in range(5)}, replay_floor_per_cell=0,
            limits=dict(max_episodes=chapters*18*6, max_observation_bytes=1000000,
                max_reply_target_bytes=1000000, max_generator_calls=100000, max_seconds=120))
        metrics = {key:dict(count=0,total=2) for key in author.METRICS}
        current = dict(weights_sha256="1"*64, lifetime_updates=1944,
                       by_family={f:deepcopy(metrics) for f in author.FAMILIES})
    ...
    def recipe(self, request):
        ...
        recipe["chapters"][-1] = dict(motif="pool_b",realization="rename")
        return recipe
    ...
    def service(self, request, *, content=None, error=None, before="3"*64, after="3"*64, done=True):
        ...
        def call(route,payload,deadline):
            ...
            self.assertEqual(set(payload["format"]["properties"]),{"schema","chapters"})
    ...
    def test_invalid_and_injected_recipes_fall_back_with_original_response(self):
        request=self.request()
        request["contract"]["chapters"][0]["motifs"]=["pool_a"]
        request["contract"]["chapters"][0]["realizations"]=["independent"]
        bad=self.recipe(request);bad["chapters"][0]["family"]="color"
        unknown=self.recipe(request);unknown["chapters"][0]["motif"]="unknown"
        wrong_position=self.recipe(request);wrong_position["chapters"][0]["motif"]="pool_b"
        self.assertIn("pool_b",author._wire_schema(request["contract"])["properties"]["chapters"]["items"]["properties"]["motif"]["enum"])
        for i,content in enumerate((json.dumps(bad),json.dumps(unknown),json.dumps(wrong_position),'{"schema":"x","schema":"y","chapters":[]}')):
    ...
    def test_parent_development_source_and_contract_rejected_before_intent(self):
        original=self.request()
        for case in ("audit","parent","source","slot","recipe","hash"):
            request=deepcopy(original)
            if case=="audit":request["development"]["role"]="audit"
            elif case=="parent":request["parent"]["weights_sha256"]="9"*64
            elif case=="source":request["source_sha256"]["experiments/verified_tutor_author_v2.py"]="0"*64
            elif case=="slot":request["contract"]["slots"].pop()
            elif case=="recipe":request["procedural_recipe"]["chapters"][0]["realization"]="rename"
    ...
    def test_local_digest_required_before_and_after_and_no_source_drift(self):
        ...
        changed=deepcopy(request["source_sha256"]);changed["experiments/verified_tutor_author_v2.py"]="0"*64
    ...
    def test_compact_six_chapter_prompt_has_only_aggregate_development(self):
        request=self.request(chapters=6)
        for index,chapter in enumerate(request["contract"]["chapters"]):
            chapter["motifs"]=["balanced"] if index<2 else ["balanced","copy_emphasis","change_emphasis"]
            chapter["realizations"]=["independent"] if index<2 else ["independent","rename","revalue"]
            chapter["default"]=dict(motif="balanced",realization="independent")
        request["procedural_recipe"]=author.compiler.procedural_recipe(request["contract"])
        value=author.validate_request(request,expected_sha256=author.request_sha256(request))
        payload=author._chat_payload(value)
        ...
        self.assertNotIn(request["contract"]["inventory_sha256"],text)
        self.assertNotIn("source_sha256",text)
        self.assertEqual(payload["options"]["num_predict"],512)
        self.assertNotIn("prefixItems",json.dumps(payload["format"]))
        wire=payload["format"]["properties"]["chapters"]
        self.assertEqual((wire["minItems"],wire["maxItems"]),(6,6))
        self.assertIs(type(wire["items"]),dict)
        teaching=json.loads(payload["messages"][1]["content"])["teaching"]
        self.assertEqual(set(teaching["motif_meanings"]),{"balanced","copy_emphasis","change_emphasis"})
        self.assertEqual(set(teaching["realizations"]),{"independent","rename","revalue"})
        self.assertIn("Balanced quantiles",teaching["motif_meanings"]["balanced"])
        large=deepcopy(request)
        for panel in ("current","reference"):
            for metrics in large["development"][panel]["by_family"].values():
                for counts in metrics.values():counts.update(count=2**63-1,total=2**63-1)
        with self.assertRaisesRegex(ValueError,"input budget"):self.invoke(large,"oversize")
