...
SCHEMA = "bic-recurrent-read-pilot-v1"
PROTOCOL = "docs/RECURRENT_READ_PILOT.md"
ARMS = ("baseline", "recurrent")
STEPS = (0, 216, 432, 648)
SEED, UPDATES, MICRO, BATCH, RATE, SECONDS = 852204001, 648, 32, 32, .003, 3600
...
def contract():
    return dict(schema=SCHEMA, arms=list(ARMS), seed=SEED, config=CONFIG,
        read_passes=dict(baseline=1, recurrent=4), updates_per_arm=UPDATES,
        ...
        first_arm="baseline at even zero-based bundle IDs; recurrent at odd IDs",
        initial_weights="Exact equal state tensors; each architecture evaluated separately at step zero",
...
def source_pins():
    from experiments import recurrent_read_data as data
    from experiments import foundation_layout_kernel as kernel, foundation_layout_evaluation as evaluation
    names = set(data.source_hashes()) | set(kernel.source_hashes()) | set(evaluation.source_hashes())
    names |= {"experiments/recurrent_read_pilot.py", "experiments/recurrent_read_student.py", "experiments/recurrent_read_screen.py",
...
def run(output, *, expected_launch_sha256):
    ...
    def model_guard(arm):
        model = models[arm]
        expected_type = SequenceStudent if arm == "baseline" else RecurrentReadStudent
        if (type(model) is not expected_type or model.config != config
                or (arm == "recurrent" and model.read_passes != 4)
            ...
            raise ValueError("fixed model architecture/placement/read count changed")
    ...
    def checkpoint(arm, step):
        ...
        payload = dict(schema=SCHEMA, launch_sha256=expected_launch_sha256, arm=arm, step=step,
            architecture="sequence-baseline" if arm == "baseline" else recurrent.ARCHITECTURE,
            read_passes=launch["contract"]["read_passes"][arm], learner=image,
    ...
    try:
        ...
        import torch
        from experiments import execution_profile as execution, recurrent_read_student as recurrent
        ...
        from experiments.sequence_student import SequenceConfig, SequenceStudent, build_sequence_student
        ...
        for arm, constructor in (("baseline", build_sequence_student), ("recurrent", build_recurrent_read_student)):
            receipt["model_construction_attempts"] += 1
            models[arm] = operation("model_construction", arm, lambda make=constructor: make(SEED, device="cuda:0", config=config))
        ...
        receipt["initial_weights_sha256"] = checkpoint_digest(models["baseline"])
        del a, b
        for arm in ARMS:
            optimizer = torch.optim.AdamW(models[arm].parameters(), lr=RATE)
            kernels[arm] = PreparedLayoutKernel(models[arm], optimizer, config=config, layout="original",
                micro_batch_size=MICRO, objective_id=OBJECTIVE,
                source_sha256={"experiments/recurrent_read_student.py": launch["source_sha256"]["experiments/recurrent_read_student.py"]})
            owners[arm] = PreparedLayoutOwner(config=config, layout="original", micro_batch_size=MICRO)
        for arm in ARMS: evaluate(arm, 0)
        for cursor, record in enumerate(manifest["training"]):
            image = operation("data_load", str(cursor), lambda: load_image(record))
            ...
            for arm in order:
                ...
                report = operation("training", f"{arm}/{cursor}", lambda: kernels[arm].step(token, deadline=deadline))
                event(dict(event="step_report", arm=arm, report=report))
                model_guard(arm)
            del image
            if kernels["baseline"].evidence != kernels["recurrent"].evidence:
