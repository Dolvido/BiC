"""Observation-only evaluation of complementary composed-operation definitions.

All-query paired action plus exact free reply is primary: the entire declared
query inventory, including both words when present, must succeed in both members.
Tested application versions/words are evidence metadata, never policy arguments.
...
from experiments.complementary_composition_curriculum import FAMILIES, PANELS, REPLIES, CONTRASTS
from experiments.sequence_student import SequenceConfig
from experiments import complementary_composition_curriculum as curriculum


SCHEMA = "bic-complementary-composition-evaluation-v1"
PRIMARY_METRIC = "all_query_pair_both"
...
_ROW_KEYS = {"episode_id", "counterfactual_group", "base_pair_sha256", "base_episode_id",
    ...
    "contrast_index", "contrast_codes"}
_QUERY_KEYS = {"query_id", "turn", "kind", "rule_version", "tested_rule_word", "target", "action", "logits",
...
def source_hashes():
    ...
    names = {"experiments/complementary_composition_evaluation.py", "experiments/composition_data.py",
...
def _row_identity(row):
    return dict(episode_id=row["id"], counterfactual_group=row["counterfactual_group"],
        ...
        contrast_index=row["recipe"]["contrast_index"], contrast_codes=deepcopy(row["recipe"]["contrast_codes"]),
...
def _inventory(records):
    return [{**{key:row[key] for key in sorted(_ROW_KEYS-{"queries"})}, "queries":[{key:q[key] for key in
        ("query_id", "turn", "kind", "rule_version", "tested_rule_word", "target")} for q in row["queries"]]} for row in records]
...
def _validate_records(records):
    ...
    for row in records:
        ...
        if (row["episode_id"] in seen or row["family"] not in FAMILIES
                or row["panel"] not in PANELS
                or type(row["contrast_index"]) is not int or not 0 <= row["contrast_index"] < len(CONTRASTS)
                or type(row["contrast_codes"]) is not list or row["contrast_codes"] != list(CONTRASTS[row["contrast_index"]])
        ...
        for query in row["queries"]:
            ...
            if (type(query["query_id"]) is not str or not query["query_id"] or query["query_id"] in identities
                    ...
                    or (query["kind"] == "unknown") != (query["rule_version"] is None)
                    or (query["kind"] == "unknown") != (query["tested_rule_word"] is None)
                    or query["tested_rule_word"] is not None and (type(query["tested_rule_word"]) is not str
                        or not query["tested_rule_word"].isascii() or not query["tested_rule_word"].isalpha()
                        or not query["tested_rule_word"].islower())
    ...
    for group in groups.values():
        ...
        keys = ("base_pair_sha256", "family", "panel", "turns", "anchor_query_id", "anchor_turn_index", "contrast_index", "contrast_codes")
        ...
        if set(a) != set(b) or any(tuple(a[k][v] for v in ("turn", "kind", "rule_version", "tested_rule_word")) != tuple(b[k][v] for v in ("turn", "kind", "rule_version", "tested_rule_word")) for k in a):
...
def score_records(records, *, expected_bank=None):
    ...
    for label, key in (("family", lambda r:r["family"]), ("panel", lambda r:r["panel"]),
                       ...
                       ("contrast", lambda r:"/".join(r["contrast_codes"])),
                       ("family_panel_contrast", lambda r:r["family"]+"/"+r["panel"]+"/"+"/".join(r["contrast_codes"]))):
...
class PreparedDefinitionBank:
    def __init__(self, rows, *, role, config=None):
        ...
        if role not in ("train_fit", "dev"):
            raise ValueError("explicit train_fit/dev role required")
        ...
        for offset in range(0, len(admitted), 2):
            ...
            for row in pair:
                ...
                inventory.append(dict(**_row_identity(row), queries=[dict(query_id=q["query_id"], turn=q["turn_index"],
                    kind=q["kind"], rule_version=q["rule_version"], tested_rule_word=q["tested_rule_word"], target=row["turns"][q["turn_index"]]["target"])
        ...
        self._identity = dict(schema=curriculum.VERSION, sha256=hashlib.sha256(canonical).hexdigest(),
            ...
            contrast_scope="Canonical contrast labels are verification metadata, never observation tensors or policy inputs.",
            revision_scope="rule_version identifies the tested application, including unchanged roles; version1 queries follow a revised application, not merely a previous entity mutation.",
            tested_rule_word_scope="Verification metadata naming each query's tested rule; neither word metadata nor targets enter neural inputs.")
                    ...
                    for member, row in enumerate(chunk):
                        ...
                        for query in sorted(row["queries"], key=lambda q: q["turn_index"]):
                            ...
                            queries.append(dict(query_id=query["query_id"], turn=turn, kind=query["kind"], rule_version=query["rule_version"],
                                tested_rule_word=query["tested_rule_word"],
