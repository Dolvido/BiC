...
SCHEMA = "bic-entity-retrieval-state-diagnostic-v1"
PROTOCOL = "docs/ENTITY_RETRIEVAL_DIAGNOSTIC.md"
...
ARMS,FAMILIES,LENGTHS = ("baseline","retrieval"),("color","count","switch"),(8,10,12)
...
def run(directory, *, main_directory,summary_sha256,launch_sha256,source_sha256,protocol_sha256):
    ...
    if (summary["status"]!="completed" or summary["launch_sha256"]!=launch_sha256
            or launch["schema"]!="bic-entity-retrieval-pilot-v1" or set(summary["arms"])!=set(ARMS)
        ...
        raise ValueError("completed final entity-retrieval run and verified metric origin required")
    ...
    try:
        import torch
        from experiments import execution_profile as execution,shared_state_student as baseline,entity_retrieval_student as retrieval
        ...
        with ExitStack() as stack:
            ...
            for arm in ARMS:
                ...
                module=baseline if arm=="baseline" else retrieval
                if (saved["schema"]!=launch["schema"] or saved["launch_sha256"]!=launch_sha256 or saved["arm"]!=arm
                        ...
                        or saved["auxiliary_weight"]!=.3 or saved["runtime"]!=receipt["runtime"]
                ...
                builder=baseline.build_shared_state_student if arm=="baseline" else retrieval.build_entity_retrieval_student
                ...
                with torch.inference_mode():
                    for key,(family,turns,inputs,labels,ids) in prepared.items():
                        ...
                        logits=(model.state_logits(output["context_states"],batch["eos_positions"]) if arm=="baseline"
                            else model.state_logits_from_output(output,batch["eos_positions"]))
                ...
                result["arms"][arm]=dict(checkpoint=record,weights_sha256=before,model_unchanged=True,
