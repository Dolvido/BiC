"""Recoverable local tutor campaign with verified-practice fallback.
...
SCHEMA = 'bic-continuous-tutor-campaign-v3'
...
PROTOCOL = 'docs/CONTINUOUS_TUTOR_CAMPAIGN_FALLBACK.md'
...
def source_hashes():
    ...
    for path in ('experiments/continuous_tutor_campaign_v3.py',
        ...
        while True:
            ...
            if stage=='ready':
                ...
                manifest=data.compile_data(cycle_dir,cycle_sha256=state['cycle_context']['sha256'],
                    teacher_decision=state['teacher_decision'],legacy_context=state.get('legacy_context'),
                    reuse_procedural=state.get('reuse_procedural'),max_seconds=required,
                    deadline=min(deadline-FINAL_RESERVE,tick+required))
                ...
                for key in ('cycle_directory','cycle_context','cycle_spec','teacher_decision','compiled','worker','legacy_context','reuse_procedural'):
                    state.pop(key,None)
...
FINITE_VARIATION_ERROR = "ValueError('fresh realization exhausted 64 candidates: slot=105, offset=14, family=switch, realization=revalue')"


def recovery_state(previous, summary, state, context, preparation):
    """Admit the known second-cycle compilation refusal, preserving completed work."""
    if (previous.get('schema') != 'bic-continuous-tutor-campaign-v2'
            or summary.get('status') != 'failed'
            or summary.get('error') != FINITE_VARIATION_ERROR
            or summary.get('partial_work_unknown') is not False
            or len(summary.get('worker_processes',[])) != 1
            or summary['worker_processes'][0].get('exit_code') != 0
            or summary.get('physical_totals') != dict(worker_updates=432,
                teacher_chat_attempts=2,teacher_chat_completions=2)
            or summary.get('active_stage',{}).get('stage') != 'authored'
            or summary.get('active_stage',{}).get('cycle') != 2
            or state.get('schema') != previous['schema']
            or state.get('stage') != 'authored' or state.get('cycle') != 2
            or state.get('completed_cycles') != 1
            or state.get('parent') != summary['parent']
            or state.get('physical_totals') != summary['physical_totals']):
        raise ValueError('only known second-cycle finite-variation refusal is recoverable')
    parent=state['parent']
    reports=preparation.get('compiler_receipts',[])
    if (context.get('parent') != dict(identity_sha256=parent['checkpoint']['sha256'],
            weights_sha256=parent['weights_sha256'],cycle=2,lifetime_updates=parent['lifetime_updates'])
            or preparation.get('status')!='failed' or preparation.get('error')!=FINITE_VARIATION_ERROR
            or preparation.get('compiler_attempts')!=2 or preparation.get('compiler_completions')!=1
            or len(reports)!=2 or reports[0].get('status')!='completed'
            or reports[1].get('status')!='failed'
            or reports[1].get('error') != 'fresh realization exhausted 64 candidates: slot=105, offset=14, family=switch, realization=revalue'
            or preparation.get('neural_work')!=0 or preparation.get('teacher_calls')!=0):
        raise ValueError('completed procedural and known rejected tutor work required')
    spent=summary['wall_seconds']
    if type(spent) not in (int,float) or not math.isfinite(spent) or spent<=0:
        raise ValueError('measured previous elapsed work required')
    remaining=math.floor(previous['budget_seconds']-spent)
    if remaining<120: raise ValueError('no remaining granted invocation budget')
    result=deepcopy(state);result['schema']=SCHEMA
    validate_state(result)
    return result,remaining


def freeze_recovery(output, *, parent_directory, parent_summary_sha256, expected_preparation_sha256):
    """Preserve the rejected author proposal and use verified practice; debit prior elapsed work."""
    started,cpu=time.monotonic(),time.process_time()
    from experiments import verified_tutor_author_v2 as author
    output,prior=Path(output).resolve(),Path(parent_directory).resolve()
    summary=_checked(prior/'execution/summary.json',parent_summary_sha256)
    old_launch_pin=digest(prior/'launch.json');previous=read(prior/'launch.json')
    if summary['launch_sha256']!=old_launch_pin: raise ValueError('failed launch identity differs')
    verify_pins(previous['source_sha256']);verify_pins(previous['input_sha256'])
    saved=summary['last_state'];state=_pin_record(saved)
    context=_pin_record(state['cycle_context'])
    old_cycle=ROOT/state['cycle_directory'];old_compiled=old_cycle/'compiled'
    prep_record=_record(old_compiled/'preparation.json')
    if prep_record['sha256'] != expected_preparation_sha256:
        raise ValueError('caller-pinned failed preparation receipt differs')
    preparation=_pin_record(prep_record)
    state,remaining=recovery_state(previous,summary,state,context,preparation)
    decision_record=state['teacher_decision']
    accepted=author.load_result(ROOT/decision_record['path'],expected_sha256=decision_record['sha256'],
        expected_request_sha256=decision_record['request_sha256'])
    if (accepted['outcome']!='local_accepted' or accepted['parent']!=context['parent']
            or accepted['contract_sha256']!=context['teaching_contract']['identity_sha256']
            or accepted['teacher_cost']['physical_teacher_work_unknown'] is not False
            or accepted['teacher_cost']['chat_attempts']!=1
            or accepted['teacher_cost']['chat_completions']!=1):
        raise ValueError('exact accepted persisted tutor result required; no new request')
    sources=source_hashes()
    pins=deepcopy(previous['input_sha256']);pins.update(previous['source_sha256'])
    for record in (saved,state['cycle_context'],prep_record,decision_record):
        pins[record['path']]=record['sha256']
    pins.update({relative_root(prior/'launch.json'):old_launch_pin,
                 relative_root(prior/'execution/summary.json'):parent_summary_sha256})
    verify_pins(pins)
    state['legacy_context']=deepcopy(state['cycle_context'])
    state['reuse_procedural']=dict(directory=relative_root(old_compiled),preparation_sha256=prep_record['sha256'])
    state['cycle_directory']=relative_root(output/f"execution/cycles/{state['cycle']:06d}")
    native(output).mkdir(parents=True,exist_ok=False)
    inherited=dict(directory=relative_root(prior),launch_sha256=old_launch_pin,
        summary_sha256=parent_summary_sha256,last_state=saved,failed_compilation=prep_record,
        wall_seconds=summary['wall_seconds'],cpu_seconds=summary['cpu_seconds'],
        physical_totals=summary['physical_totals'],freeze_cost=previous['freeze_cost'],
        compilation_cost={k:preparation[k] for k in ('wall_seconds','cpu_seconds','compiler_receipts')},
        recovery='Retain rejected teacher proposal and cost; copy verified procedural artifacts as explicit zero-contrast fallback, then compile common withdrawal.')
    launch=dict(schema=SCHEMA,created_utc=utc(),seed=previous['seed'],budget_seconds=remaining,
        final_reserve_seconds=FINAL_RESERVE,stage_seconds=STAGE_SECONDS,teacher=previous['teacher'],
        origin_identity=previous['origin_identity'],parent_runtime=previous['parent_runtime'],
        initial_state=state,resumed=False,recovery=inherited,evaluation=previous['evaluation'],
        inventory=previous['inventory'],source_sha256=sources,input_sha256=pins,
        freeze_work=dict(checkpoint_metadata_loads=0,models=0,teacher_calls=0,canonical_generation_calls=0),
        freeze_cost=dict(wall_seconds=time.monotonic()-started,cpu_seconds=time.process_time()-cpu),
        automatic_promotion=False,scope=previous['scope'])
    return publish(output/'launch.json',launch)


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('mode',choices=('freeze','recover','run'));p.add_argument('directory')
    p.add_argument('--launch-sha256');p.add_argument('--parent-directory');p.add_argument('--parent-summary-sha256')
    p.add_argument('--preparation-sha256')
    ...
    if args.mode=='freeze':
        ...
        pin=freeze_recovery(args.directory,parent_directory=args.parent_directory,
            parent_summary_sha256=args.parent_summary_sha256,expected_preparation_sha256=args.preparation_sha256)
        print(encoded(dict(launch_sha256=pin)).decode().strip())
    else:
