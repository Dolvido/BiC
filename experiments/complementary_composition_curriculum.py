"""Complementary two/three-clause meanings with fixed structural holdouts.

Independent typed and visible-English interpreters admit only C, S, D and the
five CONTRASTS. SC, CS, SCS and CSS are never accepted by this provider. These
are program-string holdouts, not universal semantic-function holdouts: switch
CSS equals C, and SCS equals CD/DCD. NONCES intentionally retain the basis
split vocabularies; this is a new provider, not a claim of globally new words.
Binding has two rules, shuffled definitions and swapped application roles;
revision retains old-state queries before applying the new rule. Query versions
identify the tested application, not the last mutation. Recipe/program/query
metadata and state labels remain outside native policy inputs.
"""
...
VERSION = "bic-complementary-composition-curriculum-v1"
...
CONTRASTS = (("CD", "DC"), ("SD", "DD"), ("SS", "DS"),
             ("DCD", "CDD"), ("SSD", "SDD"))
ATOMS = ("C", "S", "D")
FORBIDDEN_CODES = ("SC", "CS", "SCS", "CSS")
_PRIMITIVES = {"C":"copy", "S":"advance_source", "D":"advance_destination"}
MEANINGS = {code:tuple(_PRIMITIVES[c] for c in code)
            for code in ATOMS+tuple(code for pair in CONTRASTS for code in pair)}
...
def source_hashes():
    ...
    return {name:hashlib.sha256((root/name).read_bytes()).hexdigest() for name in
            ("experiments/complementary_composition_curriculum.py", "experiments/cognitive_curriculum.py")}
...
def _english_run(texts):
    ...
    for text in texts:
        ...
        if match:=_DEFINE.fullmatch(text):
            ...
            # A separate explicit grammar admission list excludes the four
            # reserved programs, even when this interpreter is called directly.
            code="".join({"copy source to destination":"C", "advance source once":"S",
                          "advance destination once":"D"}[phrase] for phrase in phrases)
            if code not in ("C","S","D","CD","DC","SD","DD","SS","DS","DCD","CDD","SSD","SDD"):
                raise ValueError("program is outside the complementary teaching grammar")
...
def _programs(family,seed,split,panel):
    # Contrast is explicit seed%5; one shared plan covers all three families.
    ...
    contrast_index=seed%5
    meanings=CONTRASTS[contrast_index]
    ...
    if panel=="revision":
        prior=rng.choice(ATOMS)
        b=_advance(family,a) if rng.randrange(2) else a
    ...
    return result,dict(word=word,other_word=None if panel=="revision" else other_word,
        contrast_index=contrast_index,contrast_codes=list(CONTRASTS[contrast_index]),
        definition_order=None if panel=="revision" else definition_order,
...
def _generate_pair(family,seed,split,panel,turns):
    ...
    for variant,program in enumerate(programs):
        ...
        if (parsed_family!=family or actions!=english_actions or labels!=english_labels or causes!=english_causes
                or len(texts)!=12 or sum(len(t.encode())+2 for t in texts)>1024
                or any(len(REPLIES[action].encode())+1>32 for action in actions)):
        ...
        rows.append(dict(version=VERSION,id=ids[variant],family=family,split=split,structure_partition=split,
            ...
            semantic_partition="complementary_program_orders",
...
def bundle_evidence(bundle):
    ...
    for family in FAMILIES:
        ...
        for index in range(0,len(rows),2):
            ...
            for row in pair:
                ...
                dimensions.add((row["depth"],len(row["turns"]),row["panel"],row["recipe"]["contrast_index"]))
    ...
    if len(dimensions)!=1:raise ValueError("one shared panel/depth/length/contrast across all families required")
