...
def analyze(directory):
    ...
    if set(summary["evaluations"]) != {"baseline", "retrieval"}:
    ...
    screen = compare(endpoints["baseline"], endpoints["retrieval"])
    ...
    extra_seconds = timing["retrieval"]["training_including_preparation"] - timing["baseline"]["training_including_preparation"]
    ...
    result = dict(status="verified", summary_sha256=digest(directory / "execution/summary.json"),
        ...
        scope="Pure recount of recorded predictions, not another model evaluation; one exploratory initialization. Both arms receive state teaching; the intervention includes retrieval/fusion parameters and computation.")
