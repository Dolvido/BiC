...
SCHEMA = "bic-entity-retrieval-pilot-v1"
PROTOCOL = "docs/ENTITY_RETRIEVAL_PILOT.md"
ARMS = ("baseline", "retrieval")
STEPS = (0, 216, 432, 648)
SEED, UPDATES, MICRO, BATCH, RATE, SECONDS = 852504001, 648, 32, 32, .003, 3600
...
def contract():
    return dict(schema=SCHEMA, arms=list(ARMS), seed=SEED, config=CONFIG,
        auxiliary_weights=dict(baseline=.3, retrieval=.3), updates_per_arm=UPDATES,
        ...
        first_arm="baseline at even zero-based bundle IDs; retrieval at odd IDs",
        initial_weights="Exact equal common tensors including shared auxiliary head; extra candidate retrieval/fusion tensors recorded; both policies evaluated at zero",
        state_targets="Computed once per authenticated bundle from causal English prefixes; labels only enter training loss",
        candidate="Causal learned alias retrieval and native policy fusion; no parsed state in forward",
...
def source_pins():
    ...
    from experiments import entity_retrieval_training as kernel, foundation_layout_evaluation as evaluation
    names = set(data.source_hashes()) | set(kernel.source_hashes()) | set(evaluation.source_hashes())
    names |= {"experiments/entity_retrieval_pilot.py", "experiments/entity_retrieval_student.py", "experiments/shared_state_student.py", "experiments/shared_state_targets.py", "experiments/shared_state_screen.py", "experiments/entity_retrieval_report.py",
...
def run(output, *, expected_launch_sha256):
    ...
    def model_guard(arm):
        model = models[arm]
        if (type(model) is not (SharedStateStudent if arm == "baseline" else EntityRetrievalStudent) or model.config != config
    ...
    def checkpoint(arm, step):
        ...
        payload = dict(schema=SCHEMA, launch_sha256=expected_launch_sha256, arm=arm, step=step,
            architecture=student.ARCHITECTURE if arm == "baseline" else retrieval.ARCHITECTURE,
    ...
    try:
        ...
        from experiments.entity_retrieval_training import EntityRetrievalKernel
        from experiments import entity_retrieval_student as retrieval
        from experiments.entity_retrieval_student import EntityRetrievalStudent, build_entity_retrieval_student
        ...
        for arm in ARMS:
            ...
            models[arm] = operation("model_construction", arm, lambda: (build_shared_state_student if arm == "baseline" else build_entity_retrieval_student)(SEED, device="cuda:0", config=config))
            receipt["model_constructions"] += 1
            model_guard(arm)
        a, b = (models[arm].state_dict() for arm in ARMS)
        if not set(a).issubset(b) or any(a[key].dtype != b[key].dtype or a[key].shape != b[key].shape
        ...
        receipt["initial_common_weights_equal"] = True
        receipt["candidate_only_state_keys"] = sorted(set(b)-set(a))
        ...
        for arm in ARMS:
            ...
            kernels[arm] = EntityRetrievalKernel(models[arm], optimizer, config=config, layout="original",
                micro_batch_size=MICRO, objective_id=OBJECTIVE,
                architecture=arm, auxiliary_weight=launch["contract"]["auxiliary_weights"][arm])
        ...
        for cursor, record in enumerate(manifest["training"]):
            ...
            if kernels["baseline"].evidence != kernels["retrieval"].evidence:
