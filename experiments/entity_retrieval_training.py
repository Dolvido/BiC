"""Original three-family update with matched causal-entity state teaching.

The caller computes canonical prefix targets once from authenticated bundle
text. Targets are supervision only and never model.forward arguments. Both architectures use weight .3; candidate auxiliary logits reuse the retrieval
graph already used by the native policy.
...
SCHEMA = "bic-entity-retrieval-training-v1"
...
def source_hashes():
    ...
    result.update(retrieval.source_hashes())
    for name in ("experiments/entity_retrieval_training.py", "experiments/shared_state_targets.py"):
        result[name] = hashlib.sha256((ROOT/name).read_bytes()).hexdigest()
    return result


class EntityRetrievalKernel(PreparedLayoutKernel):
    def __init__(self, model, optimizer, *, architecture, auxiliary_weight=.3, **kwargs):
        allowed = {"baseline": student.SharedStateStudent, "retrieval": retrieval.EntityRetrievalStudent}
        if architecture not in allowed or type(model) is not allowed[architecture] or type(auxiliary_weight) not in (int, float) or auxiliary_weight != .3:
            raise ValueError("exact declared baseline/retrieval model and fixed .3 weight required")
        self._architecture = architecture
        self._model_class = allowed[architecture]
        ...
        self._recipe.update(schema=SCHEMA, source_sha256=deepcopy(self._sources),
            architecture=student.ARCHITECTURE if architecture == "baseline" else retrieval.ARCHITECTURE,
            arm=architecture, auxiliary_weight=self._auxiliary_weight,
            auxiliary_objective="shared_state_targets.state_loss; family mean after original plus weighted state loss",
            retrieval_reuse="Candidate auxiliary logits use forward entity_states; no extra retrieval pass",
    ...
    def _guard(self):
        super()._guard()
        if hasattr(self, "_auxiliary_weight") and (type(self.model) is not self._model_class
                or self._architecture != self._recipe["arm"]
            ...
            for family in training.FAMILIES:
                ...
                if self._auxiliary_weight:
                    ...
                    state_logits = (self.model.state_logits(output["context_states"], batch["inputs"]["eos_positions"])
                        if self._architecture == "baseline" else self.model.state_logits_from_output(output, batch["inputs"]["eos_positions"]))
