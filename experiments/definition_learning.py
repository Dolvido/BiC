"""A bounded native acquisition experiment for English-defined instructions.

The exact selected campaign learner and full AdamW continue through216 updates.
New definition lessons alternate with independently admitted broad rehearsal.
The external teacher is absent throughout training and evaluation in this first
capability probe; explanations are independently verified curriculum inputs.
"""
...
SCHEMA = 'bic-definition-learning-v1'
UPDATES, MICRO = 216, 32
PROTOCOL = 'docs/DEFINITION_LEARNING_PROTOCOL.md'


def source_hashes():
    from experiments import definition_data as data, definition_evaluation as evaluation
    ...
    result = {**data.source_hashes(), **evaluation.source_hashes(), **bridge.source_hashes(),
        **banks.source_hashes(), **old_evaluation.source_hashes(), **prepared.source_hashes(), **targets.source_hashes()}
    for name in ('experiments/definition_learning.py','experiments/continuous_tutor_worker.py',PROTOCOL):
        result[name] = digest(ROOT/name)
    return result


def freeze(output, *, campaign_directory, campaign_summary_sha256,
           data_directory, data_manifest_sha256, max_seconds=600):
    started, cpu = time.monotonic(), time.process_time()
    if type(max_seconds) is not int or not 120 <= max_seconds <= 1800:
        raise ValueError('fixed120..1800-second local allowance required')
    from experiments import definition_data as data, continuous_tutor_worker as worker
    ...
    try:
        ...
        terminal_compilation_refusal=(summary['status']=='failed'
            and summary.get('error')=="ValueError('fresh realization exhausted 64 candidates: slot=105, offset=14, family=switch, realization=revalue')"
            and summary.get('active_stage',{}).get('stage')=='authored'
            and summary.get('final_stage')=='authored'
            and all(child.get('exit_code')==0 for child in summary.get('worker_processes',[])))
        if (summary['status']!='completed' and not terminal_compilation_refusal
                or summary['partial_work_unknown']
                or summary['launch_sha256']!=digest(campaign_directory/'launch.json')):
            raise ValueError('completed campaign or known pre-training refusal required before new GPU run')
        ...
        if digest(ROOT/record['path'])!=record['sha256'] or state['parent']!=summary['parent']:
            raise ValueError('selected durable learner differs')
        if terminal_compilation_refusal and (state['stage']!='authored'
                or state['cycle']!=state['completed_cycles']+1
                or state['physical_totals']!=summary['physical_totals']):
            raise ValueError('refused proposal must leave exact committed learner/controller state')
        ...
        launch=dict(schema=SCHEMA,created_utc=utc(),source_sha256=sources,source_snapshots=snapshots,input_sha256=pins,
            ...
            endpoints=[0,108,216],training_definition_episodes=10368,training_replay_episodes=10368,
            teacher_calls=0,automatic_retry=False,automatic_promotion=False,
            parent_campaign_outcome=summary['status'],parent_compilation_refusal=terminal_compilation_refusal,
            scope='First definition-grounded acquisition probe; exact native continuation and retained broad practice.')
...
def run(output, *, launch_sha256):
    ...
    definition_work_before=old_validation_work=None
    ...
    def score_bank(name,bank,*,legacy=False):
        ...
        active_ledger=(old_evaluation if legacy else definition_evaluation).EvaluationLedger()
    ...
    try:
        ...
        import torch
        from experiments import execution_profile, definition_data as data, definition_evaluation
        ...
        definition_work_before=definition_curriculum.work_report();old_validation_work=WorkLedger()
        ...
        for index in range(UPDATES):
            boundary();kind='definition' if index%2==0 else 'replay';source_index=index//2
            record=manifest['training' if kind=='definition' else 'replay'][source_index]
            if kind=='definition':
                receipt['definition_json_load_attempts']+=1
                lesson=json.loads(authenticated(data_dir/record['path'],record['sha256']))
                receipt['definition_json_loads']+=1
                if lesson['admission_schema']!=definition_curriculum.VERSION:raise ValueError('new definition admission required')
            ...
            original_cursor=record['index'] if kind=='definition' else record['original_cursor']
...
if __name__=='__main__':
    ...
    p.add_argument('--max-seconds',type=int,default=600);args=p.parse_args()
    if args.mode=='freeze':
