...
def analyze(directory):
    ...
    if set(summary["evaluations"]) != {"control", "state"}:
    ...
    screen = compare(endpoints["control"], endpoints["state"])
    ...
    extra_seconds = timing["state"]["training_including_preparation"] - timing["control"]["training_including_preparation"]
    ...
    result = dict(status="verified", summary_sha256=digest(directory / "execution/summary.json"),
        ...
        incremental_teaching_seconds_including_targets=extra_seconds + target_seconds,
