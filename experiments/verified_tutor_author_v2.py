...
SCHEMA = "bic-verified-tutor-author-v2"
REQUEST_SCHEMA = "bic-verified-tutor-author-request-v2"
...
def _wire_schema(contract):
    """Portable homogeneous items grammar; compiler enforces chapter positions."""
    value=compiler.validate_contract(contract)
    motifs=sorted({m for chapter in value["chapters"] for m in chapter["motifs"]})
    realizations=sorted({r for chapter in value["chapters"] for r in chapter["realizations"]})
    return dict(type="object",additionalProperties=False,required=["schema","chapters"],properties=dict(
        schema=dict(type="string",enum=[compiler.RECIPE_SCHEMA]),
        chapters=dict(type="array",minItems=len(value["chapters"]),maxItems=len(value["chapters"]),
            items=dict(type="object",additionalProperties=False,required=["motif","realization"],
                properties=dict(motif=dict(type="string",enum=motifs),
                                realization=dict(type="string",enum=realizations))))))


def source_hashes():
    result = {**prior.source_hashes(), **compiler.source_hashes()}
    result["experiments/verified_tutor_author_v2.py"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    return result
