"""Bounded raw-byte JSON admission cache; no learner or provider imports.
...
Every return is freshly decoded from immutable authenticated bytes, including
the first return. This removes repeated admission, not repeated JSON decoding.
...
SCHEMA = 'bic-immutable-lesson-bytes-cache-v2'
...
class ImmutableLessonBytesCache:
    ...
    def __init__(self, root, *, records, records_sha256, admission,
        ...
        for record in records:
            ...
            self._records[identity(record)] = _encoded(record)
        ...
        self._work = dict(get_attempts=0, gets=0, cache_hits=0, cache_misses=0,
            ...
            record_decode_attempts=0, record_decodes=0, admission_decodes=0, return_decodes=0,
            chunk_read_attempts=0, chunk_reads=0, max_chunk_request_bytes=0,
    ...
    def _record(self, key):
        self._work['record_decode_attempts'] += 1
        value = json.loads(self._records[key])
        self._work['record_decodes'] += 1
        return value

    def _read_bytes(self, record, *, resident=False):
        # Bound every allocation request, including on an unexpectedly huge file.
        chunks, total = [], 0
        with _native(self._path(record['path'])).open('rb') as handle:
            while total <= self.max_serialized_bytes:
                request = min(READ_CHUNK_BYTES, self.max_serialized_bytes + 1 - total)
                self._work['chunk_read_attempts'] += 1
                self._work['max_chunk_request_bytes'] = max(self._work['max_chunk_request_bytes'], request)
                chunk = handle.read(request)
                self._work['chunk_reads'] += 1
                self._work['resident_check_bytes' if resident else 'json_bytes'] += len(chunk)
                if not chunk: break
                chunks.append(chunk); total += len(chunk)
        if total > self.max_serialized_bytes: raise ValueError('lesson exceeds cache byte bound')
        raw = b''.join(chunks)
    ...
    def get(self, record):
        ...
        def operation():
            ...
            if key in self._cache:
                ...
                self._work['decodes'] += 1; self._work['admission_decodes'] += 1
                self._work['admission_attempts'] += 1
                ...
                self._cache[key] = (raw, size); self._bytes += size
            ...
            # Raw bytes already passed strict JSON/admission checks. A new C-JSON
            # decode detaches even the first return from any callback-held input.
            self._work['decode_attempts'] += 1
            result = json.loads(raw)
            self._work['decodes'] += 1; self._work['return_decodes'] += 1
            self._work['detached_copies'] += 1; self._work['gets'] += 1
            return result
        return self._timed(operation)

    def verify_sources(self):
        """Rehash resident files; decode metadata records, not lessons."""
        self._work['verify_attempts'] += 1
        def operation():
            self._guard()
            for key in self._cache:
                self._read_bytes(self._record(key), resident=True)
    ...
    def report(self):
        return dict(schema=SCHEMA, **self._work, failed=self.failed,
            ...
            max_serialized_bytes=self.max_serialized_bytes, read_chunk_bytes=READ_CHUNK_BYTES,
            representation='immutable authenticated JSON bytes',
            decode_scope='Lesson decodes include admission on misses and every return; record decodes separately counted.',
...
def source_hashes():
    return {'experiments/immutable_lesson_bytes_cache.py': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
