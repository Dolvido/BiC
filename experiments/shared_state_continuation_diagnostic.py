...
SCHEMA = "bic-shared-state-continuation-diagnostic-v1"
PROTOCOL = "docs/SHARED_STATE_CONTINUATION_DIAGNOSTIC.md"
...
def run(directory, *, main_directory,summary_sha256,launch_sha256,source_sha256,protocol_sha256):
    ...
    if (summary["status"]!="completed" or summary["launch_sha256"]!=launch_sha256
            or launch["schema"]!="bic-shared-state-continuation-pilot-v1" or summary["schema"]!=launch["schema"]
            or launch["contract"]["learning_rates"]!=RATES
            or launch["contract"]["auxiliary_weights"]!=dict(fast=.3,slow=.3)
            or launch["contract"]["lifetime_updates_per_arm"]!=1944 or launch["contract"]["original_seed"]!=852604001
            or launch["contract"]["origin_step"]!=648 or launch["contract"]["new_updates_per_arm"]!=1296 or set(summary["arms"])!=set(ARMS)
            or any(summary["arms"][arm]["cursor"]!=1944 for arm in ARMS)
            or any(origin["source_sha256"].get(name)!=pin for name,pin in METRIC_SOURCES.items())):
        raise ValueError("completed final shared-state continuation run and verified metric origin required")
    ...
    try:
        import torch
        from experiments import execution_profile as execution,shared_state_student as student,shared_state_continuation as continuation
        ...
        with ExitStack() as stack:
            ...
            for arm in ARMS:
                arm_started=time.monotonic();record=summary["checkpoints"][arm]["1944"];path=ROOT/record["path"]
                if record["path"]!=relative_root(main/f"execution/checkpoints/{arm}-1944.pt") or summary["artifact_sha256"][f"checkpoints/{arm}-1944.pt"]!=record["sha256"]:
                    raise ValueError("only predetermined final checkpoint images allowed")
                saved=load(path,record["sha256"],"checkpoint")
                bridge=saved["learner"];learner=bridge["learner"];identity=bridge["identity"]
                expected_origin=launch["origin_identities"][arm]
                if (saved["schema"]!=launch["schema"] or saved["launch_sha256"]!=launch_sha256 or saved["arm"]!=arm
                        or saved["step"]!=1944 or learner["cursor"]!=1944 or saved["architecture"]!=student.ARCHITECTURE
                        or saved["auxiliary_weight"]!=.3 or saved["learning_rate"]!=RATES[arm]
                        or saved["runtime"]!=receipt["runtime"] or learner["recipe"]["config"]!=launch["contract"]["config"]
                        or bridge["schema"]!=continuation.SCHEMA or identity["schema"]!=continuation.SCHEMA
                        or bridge["lifetime_updates"]!=1944 or bridge["continuation_updates"]!=1296
                        or bridge["bridge_cost"]["step_invocations"]!=1296
                        or bridge["weights_sha256"]!=saved["weights_sha256"]
                        or identity["origin"]!=expected_origin
                        or identity["origin_archive_sha256"]!=launch["origin_checkpoints"][arm]["sha256"]
                        or identity["source_sha256"]!=continuation.source_hashes() or identity["scope"]!=continuation.SCOPE):
                    raise ValueError("final continuation checkpoint identity/history differs")
                if (expected_origin["producing_schema"]!=continuation.PRODUCER_SCHEMA or expected_origin["arm"]!=arm
                        or expected_origin["step"]!=648 or expected_origin["learning_rate"]!=RATES[arm]
                        or expected_origin["auxiliary_weight"]!=.3 or expected_origin["runtime"]!=receipt["runtime"]
                        or expected_origin["launch_sha256"]!=launch["contract"]["origin"]["launch_sha256"]
                        or continuation._json_metadata(learner["recipe"])!=expected_origin["recipe"]):
                    raise ValueError("original rate/history or unchanged kernel recipe differs")
                continuation._metadata(learner)  # Metadata arithmetic only; no live optimizer/restorer.
                recipe=learner["recipe"]
                groups=recipe["optimizer"];stored_groups=learner["optimizer"]["param_groups"]
                ...
                model=builder(launch["contract"]["original_seed"],device="cuda:0",config=config);receipt["work"]["models"]+=1
                expected=model.state_dict();weights=learner["weights"]
                ...
                del saved,bridge,learner,identity,recipe,groups,stored_groups,weights,expected
                ...
                result["arms"][arm]=dict(checkpoint=record,learning_rate=RATES[arm],weights_sha256=before,model_unchanged=True,
                    origin_checkpoint=launch["origin_checkpoints"][arm],origin_step=648,lifetime_updates=1944,
                    continuation_updates=1296,metadata_history_verified=True,
