"""Explicit repeated-practice recovery of the local tutor/practice campaign.
...
SCHEMA = 'bic-continuous-tutor-campaign-v2'
...
PROTOCOL = 'docs/CONTINUOUS_TUTOR_CAMPAIGN_RECOVERY.md'


def source_hashes():
    from experiments import verified_tutor_cycle_data_v2 as data
    ...
    for path in ('experiments/continuous_tutor_campaign_v2.py',
...
REPEATED_PRACTICE_ERROR = "ValueError('fresh curriculum repeats an admitted prior training transcript')"


def recovery_state(previous, summary, state, context, preparation):
    """Pure admission for this known pre-training failure; no generic retry."""
    if (previous.get('schema') != 'bic-continuous-tutor-campaign-v1'
            or summary.get('status') != 'failed'
            or summary.get('error') != REPEATED_PRACTICE_ERROR
            or summary.get('partial_work_unknown') is not False
            or summary.get('worker_processes') != []
            or summary.get('physical_totals') != dict(worker_updates=0,
                teacher_chat_attempts=1,teacher_chat_completions=1)
            or summary.get('active_stage',{}).get('stage') != 'authored'
            or summary.get('active_stage',{}).get('cycle') != 1
            or state.get('schema') != previous['schema']
            or state.get('stage') != 'authored' or state.get('cycle') != 1
            or state.get('completed_cycles') != 0
            or state.get('parent') != previous['initial_state']['parent']
            or state.get('physical_totals') != summary['physical_totals']):
        raise ValueError('only exact known first-cycle pre-training novelty refusal is recoverable')
    parent=state['parent']
    if (context.get('parent') != dict(identity_sha256=parent['checkpoint']['sha256'],
            weights_sha256=parent['weights_sha256'],cycle=1,lifetime_updates=parent['lifetime_updates'])
            or preparation.get('status')!='failed' or preparation.get('error')!=REPEATED_PRACTICE_ERROR
            or preparation.get('compiler_attempts')!=2 or preparation.get('compiler_completions')!=2
            or len(preparation.get('compiler_receipts',[]))!=2
            or any(x.get('status')!='completed' for x in preparation['compiler_receipts'])
            or preparation.get('fresh_only_rejection',{}).get('condition')!='tutor'
            or preparation['fresh_only_rejection'].get('overlap_count',0)<=0
            or preparation.get('neural_work')!=0 or preparation.get('teacher_calls')!=0):
        raise ValueError('completed compiler work and unchanged author/parent evidence required')
...
def freeze_recovery(output, *, parent_directory, parent_summary_sha256, expected_preparation_sha256):
    """Reuse the committed author choice; debit all failed-attempt elapsed work."""
    started,cpu=time.monotonic(),time.process_time()
    from experiments import verified_tutor_author_v2 as author
    output,prior=Path(output).resolve(),Path(parent_directory).resolve()
    summary=_checked(prior/'execution/summary.json',parent_summary_sha256)
    ...
    state['cycle_directory']=relative_root(output/'execution/cycles/000001')
    native(output).mkdir(parents=True,exist_ok=False)
    inherited=dict(directory=relative_root(prior),launch_sha256=old_launch_pin,
        ...
        recovery='Report admitted training repeats; unchanged held-out exclusions and saved teacher recipe; copy verified procedural artifacts.')
