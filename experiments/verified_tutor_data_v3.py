"""Deterministic collision recovery using the same completed inventory and author result.
...
RECOVERY_SCHEMA = "bic-verified-tutor-data-recovery-v3"
RECOVERY_DECLARATION = "docs/VERIFIED_TUTOR_COMPILATION_RECOVERY.md"
...
def source_hashes(*, include_author=True):
    ...
    result.update(compiler.source_hashes())
    for name in ("experiments/verified_tutor_data_v3.py", "experiments/verified_tutor_data_v2.py", "experiments/verified_tutor_author.py", "docs/VERIFIED_TUTOR_RECOVERY.md", RECOVERY_DECLARATION):
...
def _prior_compilation(run):
    path = ROOT/"runs/verified-tutor-data-local/attempt-001/preparation.json"
    pin = "35b26ab660acca727dad4806ed377d8d51b1670da1c5a093e40e78978a2f386a"
    run.pin(path, pin)
    record = io.read(path)
    if (record["status"] != "failed" or record["compiler_attempts"] != 2
            or record["compiler_completions"] != 1 or record["neural_work"] != 0
            or record["teacher_calls"] != 0
            or record["error"] != "ValueError('protected or repeated fresh transcript')"):
        raise ValueError("preserved failed compilation required")
    _verify(record["source_sha256"])
    return dict(path=_relative(path), sha256=pin, wall_seconds=record["wall_seconds"],
        cpu_seconds=record["cpu_seconds"], archive_attempts=record["archive_attempts"],
        archive_completions=record["archive_completions"],
        bytes_deserialized=record["bytes_deserialized"],
        compiler_attempts=record["compiler_attempts"],
        compiler_completions=record["compiler_completions"],
        compiler_receipts=deepcopy(record["compiler_receipts"]),
        error=record["error"], teacher_calls=0, neural_work=0,
        scope="Discarded incomplete curriculum attempt, retained in total cost; same saved teacher choice, no new author request.")
...
def compile_data(output, *, inventory_directory, inventory_manifest_sha256,
    ...
    run = _Attempt(output, "compilation-collision-v3-recovery", max_seconds, progress, include_author=True)
    error = None
    try:
        with io._cpu_guard(run.work):
            from experiments import verified_tutor_author_v2 as author
            prior_author_attempt = _prior_author_attempt(run)
            prior_compilation = _prior_compilation(run)
            run.receipt["prior_compilation"] = deepcopy(prior_compilation)
            ...
            manifest = run.complete_manifest(dict(schema=SCHEMA, recovery_schema=RECOVERY_SCHEMA,
                ...
                prior_compilation=deepcopy(prior_compilation),
