"""Versioned finite-exhaustion fallback to the completed procedural stream.
...
Old inventory admission is authenticated transitively. An explicitly pinned
failed v2 attempt may contribute its completed procedural archives and known
finite-exhaustion receipt. Partial tutor images are never retained. Heldout and
fresh-exclusion rules remain unchanged; fallback has zero teaching contrast.
"""
...
from experiments import verified_tutor_cycle_data_v2 as previous

ROOT = Path(__file__).resolve().parents[1]
SCHEMA = "bic-verified-tutor-cycle-data-v1"
CONTEXT_SCHEMA = "bic-verified-tutor-cycle-context-v1"
ADAPTER_SCHEMA = "bic-verified-tutor-cycle-data-adapter-v3"
...
def source_hashes():
    ...
    names = ("experiments/verified_tutor_cycle_data.py", "experiments/verified_tutor_cycle_data_v2.py", "experiments/verified_tutor_data.py",
             "experiments/foundation_tutor_loop_data.py", "experiments/verified_tutor_cycle_data_v3.py")
...
def known_exhaustion(error, report):
    """Recognize only a fully accounted finite canonical collision failure."""
    if type(error) is not ValueError or type(report) is not dict:
        return False
    match = re.fullmatch(r"fresh realization exhausted 64 candidates: slot=(\d+), offset=(\d+), family=(color|count|switch), realization=(independent|rename|revalue)", str(error))
    if (match is None or report.get("status") != "failed" or report.get("error_type") != "ValueError"
            or report.get("error") != str(error) or report.get("max_candidates_per_fresh_pair") != 64
            or report.get("neural_work") != 0 or report.get("teacher_calls") != 0
            or report.get("partial_work_unknown", False) is not False):
        return False
    work = report.get("work", {})
    for prefix in ("canonical_generate", "fresh_candidate"):
        attempted, completed = (work.get(prefix+suffix) for suffix in ("_attempts", "_completions"))
        if type(attempted) is not int or type(completed) is not int or attempted < 64 or attempted != completed:
            return False
    slot, offset = int(match[1]), int(match[2])
    if not 0 <= slot < UPDATES or not 0 <= offset < MICRO//2:
        return False
    rejected = report.get("collision_rejections")
    if type(rejected) is not list or len(rejected) < 64:
        return False
    for attempt, record in enumerate(rejected[-64:]):
        if (type(record) is not dict or record.get("slot") != slot or record.get("pair_offset") != offset
                or record.get("family") != match[3] or record.get("realization") != match[4]
                or record.get("candidate_attempt") != attempt or record.get("replay") is not False
                or not record.get("rejected_members")):
            return False
    return True


def _proposal_fallback(run, report, decision, *, inherited=None):
    value = dict(schema="bic-verified-tutor-proposal-fallback-v1", reason="finite_fresh_realization_exhaustion",
        accepted_recipe=deepcopy(decision["recipe"]), accepted_recipe_sha256=decision["recipe_sha256"],
        applied="completed_procedural_teaching_stream", teacher_recipe_applied=False,
        failed_compilation_report=deepcopy(report), inherited_failure=inherited,
        cost_scope="Previously incurred proposal failure; not charged again." if inherited else
            "Failed proposal cost is included in the current compilation invocation.")
    run.receipt["proposal_fallback"] = deepcopy(value)
    return run.save("proposal-failure.json", value)
...
def _reuse_procedural(run, record, *, context, context_path, cycle_sha256, teacher_decision, contract):
    ...
    reports = failed.get("compiler_receipts", [])
    exhaustion = reports[-1] if len(reports) == 2 else {}
    if (failed["schema"] != SCHEMA or failed["status"] != "failed"
            or failed.get("adapter_schema") != previous.ADAPTER_SCHEMA
            or failed["source_sha256"] != context["source_sha256"]
            or failed["source_sha256"] != previous.source_hashes()
            or failed["compiler_attempts"] != 2 or failed["compiler_completions"] != 1
            or len(reports) != 2 or reports[0].get("status") != "completed"
            or not known_exhaustion(ValueError(exhaustion.get("error", "")), exhaustion)
            or failed["error"] != repr(ValueError(exhaustion["error"]))
            or failed["neural_work"] != 0 or failed["teacher_calls"] != 0
            or any(failed["work"]["guard_attempts"].values())
        ...
        raise ValueError("exact v2 completed procedural / finite exhausted proposal required")
    ...
    recovery = dict(legacy_context=dict(path=_relative(context_path), sha256=cycle_sha256),
        ...
        failed_proposal=deepcopy(exhaustion), skipped_tutor_compilation=True,
...
def compile_data(output, *, cycle_sha256, teacher_decision, max_seconds=1800,
    ...
    """Compile streams; known finite tutor exhaustion uses procedural bytes.

    Explicit v2 recovery reuses the pinned complete procedural stream and known
    failed proposal receipt. It never reruns that teacher or proposal.
    """
    ...
    try:
        with io._cpu_guard(run.work):
            ...
            if legacy_context is not None:
                ...
                wanted_sources = previous.source_hashes()
            else:
            ...
            reuse_counts, recovery, fallback, cached_procedural = {}, None, None, None
            for label, recipe, contract in (("procedural", compiler.procedural_recipe(contracts["teaching"]), contracts["teaching"]),
                ...
                if reused:
                ...
                elif label == "tutor" and recovery is not None:
                    fallback = _proposal_fallback(run, recovery["failed_proposal"], decision,
                        inherited=recovery["failed_preparation"])
                    result, use_fallback = cached_procedural, True
                else:
                    ...
                    try:
                        ...
                        report = getattr(exc, "compilation_report", None)
                        if report is not None:
                            run.receipt["compiler_receipts"].append(deepcopy(report))
                        if label != "tutor" or not known_exhaustion(exc, report):
                            raise
                        fallback = _proposal_fallback(run, report, decision)
                        result, use_fallback = cached_procedural, True
                    else:
                        run.receipt["compiler_completions"] += 1
                        run.receipt["compiler_receipts"].append(result["receipt"])
                if label == "procedural":
                    cached_procedural = result
                ...
                for index, image in enumerate(result["images"]):
                    ...
                    name = f"{'procedural' if use_fallback else label}/{index:03d}.pt"
                    pin = run.artifacts[name] if reused or use_fallback else run.save(name, image)["sha256"]
                ...
                if label == "tutor": cached_procedural = None
                del result
            tutor_pins = {k:manifests["tutor"][k] for k in ("contract_sha256", "recipe_sha256")}
            applied_recipe = compiler.procedural_recipe(contracts["teaching"]) if fallback else decision["recipe"]
            if tutor_pins != dict(contract_sha256=decision["contract_sha256"], recipe_sha256=compiler.recipe_sha256(applied_recipe)):
                raise ValueError("compiled treatment differs from the recorded applied recipe")
            ...
            return run.publish("manifest.json", dict(schema=SCHEMA, adapter_schema=ADAPTER_SCHEMA,
                recovery=recovery, proposal_fallback=fallback, parent=deepcopy(context["parent"]),
                ...
                teacher_proposal=dict(recipe=deepcopy(decision["recipe"]), recipe_sha256=decision["recipe_sha256"],
                    outcome=decision["outcome"], applied_recipe=deepcopy(applied_recipe)),
                ...
                treatment=dict(different_teaching_bundles=different, zero_treatment_contrast=different==0,
                    ...
                    accepted_local_teacher=fallback is None and decision["outcome"]=="local_accepted",
                    teacher_recipe_applied=fallback is None, procedural_fallback=fallback is not None,
