"""Paired same-architecture rate comparison on reused data; no resume or promotion.
...
SCHEMA = "bic-shared-state-rate-pilot-v1"
PROTOCOL = "docs/SHARED_STATE_RATE_PILOT.md"
ARMS = ("fast", "slow")
STEPS = (0, 216, 432, 648)
SEED, UPDATES, MICRO, BATCH, SECONDS = 852604001, 648, 32, 32, 3600
...
def contract():
    return dict(schema=SCHEMA, arms=list(ARMS), seed=SEED, config=CONFIG,
        auxiliary_weights=dict(fast=.3, slow=.3), updates_per_arm=UPDATES,
        micro_batch_size=MICRO, evaluation_batch_size=BATCH, learning_rates=dict(RATES),
        optimizer="AdamW", gradient_clip=1., objective_id=OBJECTIVE, layout="original",
        checkpoints=list(STEPS[1:]), evaluation_steps=list(STEPS), evaluation_banks=["dev", "train_fit", "retention"],
        device="cuda:0", max_seconds=SECONDS, cpu_threads=1, cpu_interop_threads=1,
        first_arm="fast at even zero-based bundle IDs; slow at odd IDs",
        initial_weights="Exact equal complete SharedStateStudent tensors, including shared auxiliary head; both policies evaluated at zero",
        state_targets="Computed once per authenticated bundle from causal English prefixes; labels only enter training loss",
        intervention="Only AdamW learning rate differs: fast .003 versus slow .0003",
        data_manifest_sha256=DATA_MANIFEST_SHA256,
        evaluation_provenance="All banks reused and previously observed; dev720, train_fit108, retention720; no fresh validation or data generation",
        automatic_retry=False, automatic_promotion=False, teacher_calls=0,
        scope="Single-seed rate comparison on reused finite English; no fresh validation, architecture promotion or general capability claim.")
...
def source_pins():
    ...
    names |= {"experiments/shared_state_rate_pilot.py", "experiments/shared_state_student.py", "experiments/shared_state_targets.py", "experiments/shared_state_screen.py", "experiments/shared_state_rate_report.py",
...
def _manifest(directory, pin):
    from experiments import entity_retrieval_data as data
    if pin != DATA_MANIFEST_SHA256:
        raise ValueError("only the already observed entity-data manifest is authorized")
    ...
    if {name: row["episodes"] for name, row in manifest["bank_inventory"].items()} != dict(dev=720, train_fit=108, retention=720):
        raise ValueError("fixed previously observed bank sizes differ")
    return manifest
...
def run(output, *, expected_launch_sha256):
    ...
    try:
        ...
        receipt["initial_full_weights_equal"] = True
        if len(set(receipt["initial_weights_sha256"].values())) != 1:
            raise ValueError("full initial checkpoint digests differ")
        receipt["parameter_counts"] = {arm: models[arm].parameter_counts() for arm in ARMS}
        del a, b
        for arm in ARMS:
            optimizer = torch.optim.AdamW(models[arm].parameters(), lr=RATES[arm])
        ...
        initial_metrics = [{name: value["metrics"] for name, value in receipt["evaluations"][arm]["0"].items()}
                           for arm in ARMS]
        if initial_metrics[0] != initial_metrics[1]:
            raise ValueError("equal initial models produced different native evaluation metrics")
        receipt["initial_native_metrics_equal"] = True
        for cursor, record in enumerate(manifest["training"]):
